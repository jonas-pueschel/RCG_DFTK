...
h1_coarse_solver(basis_c, maxiter) = function (ψ_c, ρ_c, Rres, tol)
    ...
    result = riemannian_conjugate_gradient(basis_c; ρ = ρ_c, ψ = ψ_c, 
        ...
        maxiter,
        callback,
        cost_residual, gradient = H1Gradient(basis_c),         
...
ea_coarse_solver(basis_c, maxiter) = function (ψ_c, ρ_c, Rres, tol)
    callback = (info) -> nothing
    cost_residual = CoarseGridCostResidual(ψ_c, Rres)
    is_converged = RcgConvergenceResidualMGH(tol, 0.1, ψ_c)
    result = riemannian_conjugate_gradient(basis_c; ρ = ρ_c, ψ = ψ_c, 
        cost_residual,
        maxiter,
        gradient = EAGradient(basis_c, CorrectedRelativeΛShift(μ = 0.0)),
        is_converged,
        callback,
        do_rayleigh_ritz = false,
    )
...
DFTK.@timing function two_level_riemannian_optimization(
    ...
    if (force_horizontal)
        ...
        G = [ψ_f[ik]'η[ik] for ik = 1:Nk]
        η = [η[ik] - ψ_f[ik] * G[ik] for ik = 1:Nk]
    end
...
DFTK.@timing function multilevel_riemannian_optimization(
    ...
    is_converged = RcgConvergenceResidual(tol),
    coarse_cond_tol = 0.45,
    coarse_cond_functor = (tol) -> ToleranceMinStepCoarseCondition(coarse_cond_tol, tol),
    coarse_density = RecalculateDensity(),
    coarse_model_tol = 1e-1,
    coarse_tol_functor = (tol) -> RelativeResTolerance(coarse_model_tol, tol),
    cost_residual = StandardCostResiudal(),
    gradient_functor = (basis) -> EAGradient(basis, CorrectedRelativeΛShift(μ = 0.0)),
    ...
    elseif length(basis_arr) == 2
        ml_coarse_solver = function (ψ_c, ρ_c, Rres, tol)
            ...
            is_converged = RcgConvergenceResidualMGH(tol, 0.1, ψ_c)
            result = riemannian_conjugate_gradient(basis_arr[1]; ρ = ρ_c, ψ = ψ_c, 
                maxiter = maxiter_inner,
                cost_residual = cost_residual_inner,
                gradient = gradient_functor(basis_arr[1]),
                retraction = retraction_functor(),
                is_converged,
                callback = callback_coarse,
                do_rayleigh_ritz = false
            )
            return result.ψ
        end
        ...
        ml_coarse_solver = function (ψ_c, ρ_c, Rres, tol)
            ...
            result = multilevel_riemannian_optimization(basis_arr[1:end-1]; ρ = ρ_c, ψ = ψ_c, 
                ...
                iteration_strat_fine,
                iteration_strat_coarse,
                do_rayleigh_ritz = false,
                force_horizontal
            )
            return result.ψ
