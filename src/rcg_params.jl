function inner_product_DFTK(basis, ξ, η)
    ...
    ξη = [dot(ξ[ik], η[ik]) for ik in 1:Nk] .* basis.kweights * n_cols
    return real(sum(ξη))
end
...
function calculate_gradient(ψ, Hψ, H, Λ, res, riem_grad::RiemannianGradient)
    ...
    P_r = [ Pks[ik] \ res[ik] for ik in 1:Nk]
    G1 = [ψ[ik]'P_ψ[ik] for ik in 1:Nk]
    G2 = [ψ[ik]'P_r[ik] for ik in 1:Nk]
    if (riem_grad.horizontal)
        X = [G1[ik] \ G2[ik] for ik in 1:Nk]
    else
        G2 = [G2[ik] + G2[ik]' for ik in 1:Nk]
        X = [lyap(G1[ik], -G2[ik]) for ik in 1:Nk]
    end
    g = [P_r[ik] - P_ψ[ik] * X[ik] for ik in 1:Nk]
    return g
end
...
function get_constant_shift(Σ::Number, Nk)
    return [Σ for ik in 1:Nk]
end
function get_constant_shift(Σ::AbstractArray, Nk)
    return [Σ[ik] for ik in 1:Nk]
end
...
DFTK.@timing function calculate_shift(ψ, Hψ, H, Λ, res, shift::RelativeEigsShift)
    Nk = size(ψ)[1]
    λ_min = [min(real(eigen(Λ[ik]).values)...) for ik in 1:Nk]
    return [λ_min[ik] * shift.μ * I for ik in 1:Nk]
end
...
DFTK.@timing function calculate_shift(ψ, Hψ, H, Λ, res, shift::RelativeEigsShift2)
    Nk = size(ψ)[1]
    λ_max = [max(real(eigen(Λ[ik]).values)...) for ik in 1:Nk]
    return [λ_max[ik] * shift.μ * I for ik in 1:Nk]
end
...
function calculate_gradient(ψ, Hψ, H, Λ, res, ea_grad::EAGradient)
    ...
    if ea_grad.naive_formula
        ...
        ξ0 = [ψ[ik] / (Λ[ik] - Σ[ik] * I) for ik in 1:Nk]
        Hξ0 = [Hψ[ik] / (Λ[ik] - Σ[ik] * I) for ik in 1:Nk]
        X = solve_H(H, ψ, Σ, ψ, Hψ, ea_grad.itmax, ea_grad.tol, ea_grad.Pks, ea_grad.h_solver; ξ0, Hξ0)
        return [ψ[ik] - X[ik] / (ψ[ik]'X[ik]) for ik in 1:Nk]
    else
...
DFTK.@timing function calculate_transport(
        ψ, ξ, η, τ, ψ_old, ::DifferentiatedRetractionTransport,
        ret_qr::RetractionQR;
        is_prev_dir = true
    )
    Nk = size(ψ)[1]
    Yrf = [ξ[ik] / ret_qr.R[ik] for ik in 1:Nk]
    Ge = [ψ[ik]'Yrf[ik] for ik in 1:Nk]
    skew = [LowerTriangular(deepcopy(Ge[ik])) for ik in 1:Nk]
    skew = [skew[ik] - skew[ik]' for ik in 1:Nk]
    for ik in 1:Nk
        for j in 1:size(skew[ik])[1]
            skew[ik][j, j] = 0.5 * skew[ik][j, j]
        end
    end
    return [ ψ[ik] * (skew[ik] - Ge[ik]) + Yrf[ik] for ik in 1:Nk]
end
...
DFTK.@timing function calculate_transport(
    ...
    Nk = size(ψ)[1]
    if is_prev_dir
        DSf = [τ * ret_pol.Sfinv[ik] * (ξ[ik]'ξ[ik]) for ik in 1:Nk]
    else
        Mtx_rhs = [η[ik]'ξ[ik]  for ik in 1:Nk]
        Mtx_rhs = τ * [Mtx_rhs[ik]' + Mtx_rhs[ik] for ik in 1:Nk]
        DSf = [lyap(ret_pol.Sf[ik], - Mtx_rhs[ik]) for ik in 1:Nk]
    end
    return [(ξ[ik] - ψ[ik] * DSf[ik]) * ret_pol.Sfinv[ik] for ik in 1:Nk]
end
...
DFTK.@timing function calculate_transport(
        ψ, ξ, η, τ, ψ_old, ::L2ProjectionTransport,
        ::AbstractRetraction;
        is_prev_dir = true
    )
    Nk = size(ψ)[1]
    G = [ψ[ik]'ξ[ik] for ik in 1:Nk]
    G = 0.5 * [G[ik]' + G[ik] for ik in 1:Nk]
    return [ξ[ik] - ψ[ik] * G[ik] for ik in 1:Nk]
end
...
DFTK.@timing function calculate_transport(
        ψ, ξ, η, τ, ψ_old, riem_proj::RiemannianProjectionTransport,
        ::AbstractRetraction;
        is_prev_dir = true
    )
    Nk = size(ψ)[1]
    ...
    P_ψ = [ Pks[ik] \ ψ[ik] for ik in 1:Nk]
    Mtx_lhs = [ψ[ik]'P_ψ[ik] for ik in 1:Nk]
    Mtx_rhs = [ψ[ik]'ξ[ik] for ik in 1:Nk]
    Mtx_rhs = [Mtx_rhs[ik]' + Mtx_rhs[ik] for ik in 1:Nk]
    X = [lyap(Mtx_lhs[ik], - Mtx_rhs[ik]) for ik in 1:Nk]
    return [ξ[ik] - P_ψ[ik] * X[ik] for ik in 1:Nk]
end
...
DFTK.@timing function calculate_transport(
        ψ, ξ, η, τ, ψ_old, ea_proj::EAProjectionTransport,
        ::AbstractRetraction;
        is_prev_dir = true
    )
    Nk = size(ψ)[1]
    Mtx_lhs = [ψ[ik]'ea_grad.Hinv_ψ[ik] for ik in 1:Nk]
    Mtx_rhs = [ψ[ik]'ξ[ik] for ik in 1:Nk]
    Mtx_rhs = [Mtx_rhs[ik]' + Mtx_rhs[ik] for ik in 1:Nk]
    X = [lyap(Mtx_lhs[ik], - Mtx_rhs[ik]) for ik in 1:Nk]
    return [ξ[ik] - ea_proj.ea_grad.Hinv_ψ[ik] * X[ik] for ik in 1:Nk]
end

struct InverseRetractionTransport <: AbstractTransport end

DFTK.@timing function calculate_transport(
        ψ, ξ, η, τ, ψ_old, ::InverseRetractionTransport,
        ::RetractionQR;
        is_prev_dir = true
    )
    if (!is_prev_dir)
    ...
    Mtx_lhs = [ψ[ik]'ψ_old[ik] for ik in 1:Nk]
    R_ir = [tsylv2solve(Mtx_lhs[ik]) for ik in 1:Nk]
    return [- ψ_old[ik] * R_ir[ik] + ψ[ik] for ik in 1:Nk]
end
...
DFTK.@timing function calculate_transport(
    ...
    Mtx_lhs = [ψ[ik]'ψ_old[ik] for ik in 1:Nk]
    S_ir = [lyap(Mtx_lhs[ik], - 2.0 * Matrix{ComplexF64}(I, size(ψ)[1], size(ψ)[1])) for ik in 1:Nk]
    return [- ψ_old[ik] * S_ir[ik] + ψ[ik] for ik in 1:Nk]
end

struct NoTransport <: AbstractTransport end

DFTK.@timing function calculate_transport(
        ψ, ξ, η, τ, ψ_old, ::NoTransport,
        ::AbstractRetraction;
        is_prev_dir = true
    )
    return ξ
...
function init_β(gamma, res, grad, param::ParamRestart)
    return init_β(gamma, res, grad, param.cg_param)
end
...
struct ExactHessianStep <: AbstractStepSize
    ...
    function ExactHessianStep(basis::PlaneWaveBasis{T}) where {T}
        ...
        occupation = [filled_occ * ones(T, n_bands) for kpt in basis.kpoints]
...
function reset_rule!(rule::ModifiedSecantRule)
    ...
    return rule.val_zero = nothing
end

function update_rule!(next, rule::ModifiedSecantRule)
    return reset_rule!(rule::ModifiedSecantRule)
end
